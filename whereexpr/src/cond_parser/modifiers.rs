...
pub(crate) fn parse(text: &str) -> Result<(Modifiers, usize), Error> {
    ...
    let start = trimmed
        .rfind('{')
        .ok_or_else(|| Error::UnmatchedModifierBracket(close_pos as u32, (close_pos + 1) as u32, text.to_string()))?;
    ...
    if content.trim().is_empty() {
        return Err(Error::EmptyModifierBlock(start as u32, (close_pos+1) as u32, text.to_string()));
    }
    ...
    for modifier in content.split(',') {
        ...
        match trimmed_modifier {
            ...
            _ => {
                return Err(Error::UnknownModifier(modifier_start as u32, modifier_end as u32, text.to_string()));
            }
        }
